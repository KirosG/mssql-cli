...
sudo yum repolist
sudo yum update
sudo yum install -y gcc git rpm-build rpm-devel rpmlint make bash coreutils diffutils patch rpmdevtools python3 libffi-devel python3-devel openssl-devel libunwind virtualenv

# Clean output dir.
rm -rf ~/rpmbuild
rm -rf ${REPO_PATH}/../rpm_output

rpmbuild -v -bb --define "_build_id_links none" --clean ${REPO_PATH}/build_scripts/rpm/mssql-cli.spec
